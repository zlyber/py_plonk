...
class Randomness:
    def __init__(self, blind_poly: List[field]):
    ...
    @classmethod
    def rand(cls, hiding_bound, params):
...
class Commitment:
    ...
    def commit(cls,powers,polynomial:list[field],hiding_bound,params):
        ...
        if hiding_bound:
            randomness = Randomness.rand(hiding_bound,params)
...
def opening_challenges(opening_challenge: field, pow):
...
def compute_witness_polynomial(p: List[field], point: field, randomness: Randomness):
    neg_p = point.neg()
    one = point.one()
...
def open_with_witness_polynomial(
    ...
    w = MSM(powers[0][num_leading_zeros:],witness_coeffs,point.params)
    random_v = None
    if hiding_witness_polynomial is not None:
        ...
        random_commit = MSM(powers[1],random_witness_coeffs,point.params)
